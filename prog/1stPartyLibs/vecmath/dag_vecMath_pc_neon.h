...
VECTORCALL VECMATH_FINLINE vec4f __builtin_shufflevector(vec4f v1, vec4f v2, int a, int b, int c, int d)
{
  float v1f[4] = {};
  float v2f[4] = {};
  vst1q_f32(v1f, v1);
...
VECTORCALL VECMATH_FINLINE int32x2_t v_make_vec2i(int x, int y)
{
  int data[2] = {x, y};
...
{
  static const int tmp[4] = {1, 2, 4, 8};
  static const vec4i movemask = vld1q_s32(tmp);
...
#else
#define v_slli(v, bits) vshlq_n_u32(v, bits)
#define v_srli(v, bits) vshrq_n_u32(v, bits)
...
{
  int64_t c = 0; //(int64_t)vget_low_s64(bits);
  vst1_s64(&c, vget_low_s64(bits));
  if (c & ~31)
    return v_zero();
